...
const defaultConfig = {
  ...
  model: process.env.REACT_APP_AI_MODEL || 'GLM-4.5-Flash'
};
...
class AIService {
  ...
  async streamExplanation(text, handlers = {}, options = {}) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const result = await window.electronAPI.performAIStream(requestData, this.config.apiUrl, this.config.apiKey);
