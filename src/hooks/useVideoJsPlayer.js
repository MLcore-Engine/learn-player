...
const useVideoJsPlayer = ({
  ...
  const cleanedTokenRef = useRef(0);
  const unbindEventsRef = useRef(null);
  const initTokenRef = useRef(0);

  const cleanupPlayer = useCallback(() => {
    if (cleanedTokenRef.current === activeTokenRef.current) {
      return;
    }
    cleanedTokenRef.current = activeTokenRef.current;
  ...
  useLayoutEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        activeTokenRef.current = initToken;
